...
if (field.type.includes('signature')) {
    // נניח שגבולות התיבה הם אלו שראינו בתמונה הקודמת
    const leftPosition = 170;  // ערך מתוקן למיקום X, ננסה למקם במרכז התיבה
    const topPosition = 200;   // ערך מתוקן למיקום Y בהתאמה לגובה התיבה

    // התאמת גודל החתימה כך שתתאים לגודל התיבה
    const width = 150;   // רוחב התיבה - ניתן לכוונן
    const height = 50;   // גובה התיבה - ניתן לכוונן

    // הצבת שדה החתימה בתוך התיבה
    page.drawRectangle({
        x: leftPosition,
