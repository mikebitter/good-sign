...
function saveSignatureAsPNG(left, top, width, height, fileName, signatureBase64) {
    ...
    if (signatureBase64) {
        ...
        loadImage(signatureBase64).then((image) => {
            ...
            const pngFileName = `fixed_signature.png`;
            const outputPath = path.join(DOCUMENTS_DIR, pngFileName);
            ...
            out.on('finish', () => {
                console.log(`Signature PNG saved successfully as: ${pngFileName}`);
                const fileStats = fs.statSync(outputPath);
                console.log(`Saved PNG size: ${fileStats.size} bytes`);
            });
